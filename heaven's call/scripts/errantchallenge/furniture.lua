        ...
        if explosion then
            ...
            if true then
                for i=0, mod.JConst.nCloudRingProjectiles do
                    --Ring projectiles:
                    local tear = Isaac.Spawn(EntityType.ENTITY_PROJECTILE, ProjectileVariant.PROJECTILE_NORMAL, 0, entity.Position, Vector(5,0):Rotated(i*360/mod.JConst.nCloudRingProjectiles), entity):ToProjectile()
...
function mod:EverchangerSolarStateUpdate(entity, data, sprite, room)
    ...
    elseif sprite:IsFinished("End1_5") then
        ...
        local ever = Isaac.Spawn(Isaac.GetEntityTypeByName(" Everchanger "), Isaac.GetEntityVariantByName(" Everchanger "), 0, room:GetDoorSlotPosition(DoorSlot.DOWN0), Vector.Zero, nil)
