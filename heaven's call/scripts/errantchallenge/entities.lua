...
--FAKER
mod.EntityFakerData = {
    ID = Isaac.GetEntityTypeByName("Fake Isaac (HC)"),
    VAR = Isaac.GetEntityVariantByName("Fake Isaac (HC)"),
    SUB = 161,
}
...
function mod:TriggerCircle(entity, tipo, trinket, fleshcorpse)
    ...
    if tipo == 4 and not mod:IsRoomElectrified() then
        specialFlag = false
    end
...
function mod:ShopkeeperUpdate(entity)
    ...
    if data.SackHead then
        if not data.Init then
            ...
            sprite:Load("gfx/entity_SaturnShopkeeper.anm2", true)
            ...
            if door then
                ...
                if flags.password ~= "1111" then
                    ...
                    if targetroomdesc.VisitedCount == 0 then
                        local everchanger = Isaac.FindByType(mod.EverchangerData.ID, mod.EverchangerData.VAR, 0)[1]
...
function mod:GuppyInit(entity)
    ...
    if entity.SubType == 160 then
        ...
        for _, poof in ipairs(Isaac.FindByType(EntityType.ENTITY_EFFECT, EffectVariant.POOF1)) do
            poof:Remove()
        end
    end
end
...
function mod:BackgroundPlanetUpdate(entity)
    if entity.SubType == mod.EntityCircleData.SUB+6 then
        ...
        if not data.Init then
            ...
            if tipo == 0 then
                sprite:Load("gfx/backdrop/planets/mercury.anm2", true)
            elseif tipo == 1 then
                sprite:Load("gfx/backdrop/planets/venus.anm2", true)
            elseif tipo == 2 then
                sprite:Load("gfx/backdrop/planets/terra.anm2", true)
            elseif tipo == 3 then
                sprite:Load("gfx/backdrop/planets/mars.anm2", true)
            elseif tipo == 4 then
                sprite:Load("gfx/backdrop/planets/jupiter.anm2", true)
            elseif tipo == 5 then
                sprite:Load("gfx/backdrop/planets/saturn.anm2", true)
            elseif tipo == 6 then
                sprite:Load("gfx/backdrop/planets/uranus.anm2", true)
            elseif tipo == 7 then
                sprite:Load("gfx/backdrop/planets/neptune.anm2", true)
            end
...
function mod:lerpAngle(angle1, angle2, t)
...
function mod:SolLightUpdate(entity)
    if entity.SubType == mod.EntityCircleData.SUB+12 then
        ...
        if not data.EndFlag and entity.SpriteScale.X >= 2 then
            ...
            Isaac.GetPlayer(0).ControlsCooldown = 180
        end

        if entity.SpriteScale.X >= 5 then
