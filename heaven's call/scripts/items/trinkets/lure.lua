...
--ITEM QUALITY
mod:AddResetFlag(ModCallbacks.MC_POST_GAME_STARTED, "savedatarun.DarkLureSeeds", {})


local darken = function (pickup, forced)
    for i, seed in ipairs(mod.savedatarun().DarkLureSeeds) do
        if seed == pickup.InitSeed or forced then
    ...
    if room:IsFirstVisit() then
        ...
        if lure_flag then
            for i, item in ipairs(Isaac.FindByType(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE)) do
                ...
                if quality < 4 then
                    table.insert(mod.savedatarun().DarkLureSeeds, item.InitSeed)
